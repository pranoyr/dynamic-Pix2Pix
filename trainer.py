...
class Trainer():
    def __init__(self, cfg, generator, discriminator, device):
        ...
        if self.cfg.TRAIN.DISTRIBUTED:
            self.G = torch.nn.parallel.DistributedDataParallel(
                generator, device_ids=[int(os.environ["LOCAL_RANK"])], broadcast_buffers=False)
            self.D = torch.nn.parallel.DistributedDataParallel(discriminator, device_ids=[
                                                               int(os.environ["LOCAL_RANK"])], broadcast_buffers=False)
        else:
            self.G = generator
            self.D = discriminator

        self.patch = (1, self.cfg.DATA.IMG_SIZE // 2 **
                      4, self.cfg.DATA.IMG_SIZE // 2 ** 4)
    ...
    def prepare_data(self):
        ...
        train_dataset = ImageDataset(
            self.cfg.DATA.ROOT_DIR, transforms=transform, mode='train')
        val_dataset = ImageDataset(
            self.cfg.DATA.ROOT_DIR, transforms=transform, mode='val')
        self.train_loader = torch.utils.data.DataLoader(
            train_dataset, batch_size=self.cfg.DATA.BATCH_SIZE, shuffle=True, num_workers=self.cfg.DATA.NUM_WORKERS)
        self.val_loader = torch.utils.data.DataLoader(
            val_dataset, batch_size=self.cfg.DATA.BATCH_SIZE, shuffle=True, num_workers=self.cfg.DATA.NUM_WORKERS)
        print("train dataset size: ", len(train_dataset))

    def get_training_config(self):
        if self.cfg.TRAIN.OPTIMIZER.NAME == 'adam':
            self.optimizer_G = torch.optim.Adam(
                self.G_without_ddp.parameters(), lr=self.cfg.TRAIN.BASE_LR, betas=(0.5, 0.999))
            self.optimizer_D = torch.optim.Adam(
                self.D_without_ddp.parameters(), lr=self.cfg.TRAIN.BASE_LR, betas=(0.5, 0.999))
    ...
    def adjust_optim(self, step):
        ...
        if step >= decay_epochs:
            ...
            lr = self.cfg.TRAIN.BASE_LR * \
                (1 - (step - decay_epochs) / (self.cfg.TRAIN.EPOCHS - decay_epochs))
    ...
    def sample_images(self):
        ...
        realA = self.inverse_transform(real_A[0]).permute(
            1, 2, 0).detach().cpu().numpy()
        fake_B = self.inverse_transform(fake_B[0]).permute(
            1, 2, 0).detach().cpu().numpy()
    ...
    def data_cyc(self, batch):
        ...
        valid = torch.ones((real_A.size(0), *self.patch)
                           ).to(self.device, non_blocking=True)
        fake = torch.zeros((real_A.size(0), *self.patch)
                           ).to(self.device, non_blocking=True)
    ...
    def noise_cycle(self, batch):
        ...
        noise_input = np.random.uniform(-1, 1, (batch["A"].size(
            0), 3, self.cfg.DATA.IMG_SIZE, self.cfg.DATA.IMG_SIZE))
    ...
    def fit(self):
        ...
        for epoch in range(self.start_epoch, self.cfg.TRAIN.EPOCHS):
            ...
            if epoch % self.cfg.LOG_FREQ == 0:
                ...
                wandb.log({'loss_D': metrics["loss_D"], 'loss_G': metrics["loss_G"],
                          'loss_pixel': metrics["loss_pixel"], 'loss_GAN': metrics["loss_GAN"]}, step=epoch)
